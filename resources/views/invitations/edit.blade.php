...
@section('title', $title ?? 'Edit Invitation')
                ...
                <form method="POST" action="{{ $updateRoute  }}">
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <select name="guest_id" id="guest_id" class="form-control" required>
                            ...
                            @forelse($guests as $guest)
                                <option value="{{ $guest->id }}" {{ (old('guest_id', $record->guest_id) == $guest->id) ? 'selected' : '' }}>
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <select name="is_attending" id="is_attending" class="form-control">
                            ...
                            <option value="1" {{ old('is_attending', $record->is_attending) ? 'selected' : '' }}>Attending</option>
                            <option value="0" {{ old('is_attending', $record->is_attending) === '0' ? 'selected' : '' }}>Not Attending</option>
                        </select>
                        @error('is_attending')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
